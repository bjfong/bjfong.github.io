...
<section class="title-footer">
		...
		<ul class="title-links">
			...
			<li><a href="//github.com/canjs/canjs/tree/v3.2.0/docs/can-guides/introduction/technical.md">source</a></li>
...
<p>The <a href="../can-core.html" title="The best, most hardened and generally useful libraries in CanJS.">core</a> of CanJS, gzipped, is under 50KB. While there may be smaller architectural libraries, there are not other frameworks that provide comparable functionality. For custom elements, observables, live binding, routing, a model layer with intelligent caching and real-time support, 50KB is very small.</p>
<p>jQuery 3.1 is 30KB minified and gzipped, and that is only providing DOM utilities. CanJS implements it’s own DOM utilities, in addition to much more.</p>
<p>Ember 2.9 is 108KB minified and gzipped, providing a comparable feature set.</p>
<p>React 15.3 is 44KB minified and gzipped, yet React is, on it’s own, simply a View layer.</p>
...
each with it’s own npm package and <a href="http://semver.org/">semantic version</a> number.
...
<p>There's a bunch of other ways that CanJS makes it easy to develop outside the box:</p>
<ul>
...
are used to transform a set of observable values into another observable value.  While event stream libraries are able to setup more complex transformations, computes can setup simple but common transformations more easily.</p>
...
<p>In event stream libraries or other computed computed libraries, you must declare your
dependencies like:</p>
...
<p><a href="../can-compute.html" title="Create an observable value.">can-compute</a> infers their own dependencies without needing to explicitly declare them, therefore requiring less boilerplate code. This means you can write <code>fullName</code> like:</p>
...
and <a href="../can-define-stream.stream.html" title="Define a property value from a stream of values.">stream</a> behaviors.</p>
...
this can easily lead to bugs.  Instead, the following uses <a href="../can-define-stream.html" title="Define property values using streams.  can-stream is used internally
to provide the stream functionality.">can-define-stream</a> and
...
<li><a href="../can-define-stream.stream.html" title="Define a property value from a stream of values.">streamed properties</a></li>
</ul>
...
the <a href="../can-define-stream.html" title="Define property values using streams.  can-stream is used internally
to provide the stream functionality.">can-define-stream</a> module adds the ability to work with event
...
<p>All off these parts, <em>Custom Elements</em>, <em>Models</em>,
<em>View Models</em>, and <em>Views</em>, are <strong>mostly</strong> written using just
two and a half APIs:</p>
<ul>
...
code in the <a href="todomvc.html" title="This guide walks through building a slightly modified version of TodoMVC with CanJS's Core libraries and can-fixture. It takes about 1 hour to complete.">TodoMVC Guide</a>.  We'll organize code related to the <code>&lt;todo-list&gt;</code> component
...
section, we created the <a href="todomvc.html" title="This guide walks through building a slightly modified version of TodoMVC with CanJS's Core libraries and can-fixture. It takes about 1 hour to complete.">TodoMVC Guide</a>'s <code>&lt;todo-list&gt;</code>'s Model, ViewModel, and View code.
...
<p>Checkout these tests running in <a href="http://justinbmeyer.jsbin.com/xaduto/edit?html,js,output">this JSBin</a>.</p>
<h3>Flexible</h3>
...
on the observable.</p>
<p><a href="http://jsbin.com/vivowu/edit?html,js,output">This JSBin</a> shows an analog clock that uses the <a href="https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API">Canvas API</a>. The <code>Analog</code>
...
<a href="../can-define-stream.html" title="Define property values using streams.  can-stream is used internally
to provide the stream functionality.">can-define-stream</a> from events dispatched on the ApplicationViewModel.</li>
...
<p>CanJS applications can be represented by <a href="https://en.wikipedia.org/wiki/UML_state_machine#Hierarchically_nested_states">Hierarchical State Machines</a>. The <a href="atm.html" title="This guide walks through building and testing an ATM application with CanJS's
Core libraries.  It teaches how to do test driven development (TDD)
...
<p>Lets explore these benefits more in the following sections:</p>
...
<p>The
following shows inspecting the <a href="todomvc.html" title="This guide walks through building a slightly modified version of TodoMVC with CanJS's Core libraries and can-fixture. It takes about 1 hour to complete.">TodoMVC Guide</a>'s <em>&quot;What needs to be done?&quot;</em> input element.  Notice how it's easy to tell that it's behavior is provided by the
...
<p><a href="http://jsbin.com/yifopus/edit?html,css,js,output">This JSBin</a> lets a user drag an item
...
<script type="text/javascript">
  var docObject = {"src":{"path":"docs/can-guides/introduction/technical.md"},"description":"Learn about CanJS's technical accomplishments. \n","name":"guides/technical","title":"Technical Highlights","type":"page","parent":"guides/introduction","order":4,"outline":"2","comment":" ","pathToRoot":"../.."};
</script>
</article>
