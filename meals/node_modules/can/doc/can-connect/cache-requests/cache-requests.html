...
<head>
	<meta charset="utf-8">
	<title>CanJS - can-connect/cache-requests/cache-requests</title>
...
<div class="breadcrumb">
  ...
  <section class="title">
	<div class="page-type">
		<h1>can-connect/cache-requests/cache-requests</h1>
		<div>module</div>
	</div>
	
  
	<section class="description">
    <p>Caches reponse data and uses it to prevent future requests or make future requests smaller.</p>
...
<section class="title-footer">
		...
		<ul class="title-links">
			...
			<li><a href="//github.com/canjs/can-connect/tree/v1.0.14/cache-requests/cache-requests.js#L7">source</a></li>
...
<div class="signature">
  ...
  <p>Overwrites <a href="cache-requests.getListData.html" title="Only request data that hasn't already been loaded by base.cacheConnection.">getListData</a> to use set logic to
determine which data is already in <a href="../base/base.cacheConnection.html" title="A connection used for caching.">cacheConnection</a> or needs to be loaded from the base connection.</p>
<p>It then gets data from the cache and/or the base connection, merges it, and returns it.</p>
    ...
    <section class="body">
    <h2>Use</h2>
<p>Use <code>cache-requests</code> in combination with a cache
like <a href="../data/memory-cache/memory-cache.html" title="Saves raw data in JavaScript memory that disappears when the page refreshes.">can-connect/data/memory-cache/memory-cache</a> or <a href="../data/localstorage-cache/localstorage-cache.html" title="Saves raw data in localStorage.">can-connect/data/localstorage-cache/localstorage-cache</a>.  For example,</p>
...
<p>This will make it so response data is cached in memory.  For example, if
today's todos are loaded:</p>
...
<p>The original request's data will be used.</p>
<h2>Using Algebra</h2>
<p><code>cache-requests</code> can also &quot;fill in&quot; the data the cache is mising if you provide
it the necessary <a href="https://github.com/canjs/can-set">set algebra</a>.</p>
...
<p>... with the appropriate configuration, <code>cache-requests</code> will only request <code>{start: 11, end: 20}</code>.
That configuration looks like:</p>
<pre><code>var algebra = new set.Algebra( set.comparators.range(&quot;start&quot;,&quot;end&quot;) );
...
<p>Notice that <code>cacheConnection</code>s often share many of the same options as the
